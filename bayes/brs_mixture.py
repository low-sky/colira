...
pp = PdfPages(figdir+'mixturemodel.pdf')
...
while not it.finished:
    ...
    if len(sub)>10:
        ...
        p0[:,4] = np.abs((np.random.randn(nwalkers)))*0.02
        p0[:,5] = np.abs(np.random.randn(nwalkers))*100
        sampler = emcee.EnsembleSampler(nwalkers, ndim, logprob3d_xoff_scatter_mixture, 
...
#######################
        ...
        t['var'][ctr] = np.median(sampler.flatchain[:,3])
        t['xoff'][ctr] = np.median(sampler.flatchain[:,2])
        t['Pbad'][ctr] = np.median(sampler.flatchain[:,4])
    it.iternext()
...
t2.write('ratios_mixmod.tex',format='latex')
