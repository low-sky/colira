...
t = Table(names=('Name','phi','phi+','phi-',\
                     'R32-','R32','R32+','var'),\
              dtypes=('S7','f8','f8','f8','f8','f8','f8','f8'))
for tag in t.keys():
    if tag != 'Name':
        t[tag].format = '{:.3f}'
...
while not it.finished:
    ...
    if len(sub)>1:
        ...
        sampler = emcee.EnsembleSampler(nwalkers, ndim, logprob2d, 
        ...
        t['R32'][ctr] = np.median(r21)
        t['R32+'][ctr] = scipy.stats.scoreatpercentile(r21,85)
        t['R32-'][ctr] = scipy.stats.scoreatpercentile(r21,15)
...
emptystring[:]=''
t2.write('ratios32.tex',format='latex')
